"""
Reinforcement Learning Agent for Celebrity Matcher using Thompson Sampling.

This module implements a Contextual Multi-Armed Bandit approach where:
- Each celebrity is an "arm"
- User facial features provide context
- Feedback (like/dislike) provides rewards
...
class CelebrityState:
    ...
    def mean_probability(self) -> float:
        """Expected success probability (mean of Beta distribution)"""
        return self.alpha / (self.alpha + self.beta)
    
    @property
    def uncertainty(self) -> float:
        """Uncertainty in success probability (variance of Beta distribution)"""
...
@dataclass
class RLMetrics:
    """Tracks RL agent performance metrics"""
    ...
    @property
    def ctr(self) -> float:
        """Click-through rate (feedback rate)"""
...
class ThompsonSamplingAgent:
    ...
    def __init__(
        ...
        Args:
            alpha_prior: Prior for successes (optimistic initialization)
            beta_prior: Prior for failures
            context_weight: Weight of context similarity in sampling
            exploration_bonus: Bonus for under-explored celebrities
            decay_rate: Decay factor for old feedback (1.0 = no decay)
            min_impressions: Minimum impressions before full trust
        """
    ...
    def register_celebrity(self, celebrity_id: int, celebrity_name: str):
        """Register a new celebrity (arm) in the bandit"""
    ...
    def _compute_context_affinity(
        ...
        """
        Compute affinity between current context and celebrity's historical contexts.
        
        Returns a multiplier in range [1 - context_weight, 1 + context_weight]
        """
        ...
        return max(0.5, min(1.5, affinity))  # Clip to reasonable range
    
    def _compute_exploration_bonus(self, celebrity_id: int) -> float:
        """
        Compute exploration bonus for under-explored celebrities.
        
        Returns a bonus in range [0, exploration_bonus]
        """
        if celebrity_id not in self.celebrity_states:
        ...
        # More bonus for less explored celebrities
    ...
    def select_celebrities(
        ...
        """
        Select top-K celebrities using Thompson Sampling.
        
        Args:
            candidate_celebrities: List of (id, name, base_probability) tuples
            context: Optional context vector (facial embedding)
            top_k: Number of celebrities to select
        
        Returns:
            List of (id, name, adjusted_probability, is_exploration) tuples
        """
        ...
        for celeb_id, celeb_name, base_prob in candidate_celebrities:
            ...
            samples.append((celeb_id, celeb_name, sampled_prob, base_prob, is_exploration))
    ...
    def update(
        ...
        """
        Update agent based on feedback (reward).
        
        Args:
            celebrity_id: ID of celebrity that received feedback
            reward: +1 for like, -1 for dislike, 0 for no feedback
    ...
    def get_celebrity_stats(self, celebrity_id: int) -> Optional[Dict]:
        """Get statistics for a specific celebrity"""
        if celebrity_id not in self.celebrity_states:
            return None
    ...
    def get_top_celebrities(self, top_k: int = 10) -> List[Dict]:
        """Get top-K celebrities by mean probability"""
        celebrities = []
    ...
    def load_from_feedback(self, feedbacks: List[Dict]):
        """
        Initialize agent state from historical feedback.
        
        Args:
            feedbacks: List of dicts with 'celebrity_id', 'celebrity_name', 'feedback_type'
        """
